...
(define (eval-lambda expr env) 
  (list 'closure (cadr expr) (caddr expr) env (make-hash)))
...
(define (eval-call expr env) 
  (mini-apply-memoized (mini-eval (cadr expr) env) (mini-eval (caddr expr) env)))

(define (mini-apply-memoized closure argval)
  (let ((cache (car (cdr (cdddr closure)))))
    (if (hash-has-key? cache argval)
        (hash-ref cache argval)
        (let ((answer (mini-apply closure argval)))
          (hash-set! cache argval answer)
          answer))))
